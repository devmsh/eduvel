				...
				<div class="row">
							...
							<div id="accordion_lessons" role="tablist" class="add_bottom_45">
								...
								@foreach($array_count_values as $array_count_key => $array_count_value)
									...
									<div class="card">
										<div class="card-header" role="tab" id="heading{{ $number }}">
											<h5 class="mb-0">
												<a data-toggle="collapse" href="#collapse{{ $number }}" aria-expanded="true" aria-controls="collapse{{ $number }}"><i class="indicator ti-plus {{-- minus --}}"></i> {{ $array_count_key }}</a>
											</h5>
										</div>

										<div id="collapse{{ $number }}" class="collapse {{-- show --}}" role="tabpanel" aria-labelledby="heading{{ $number }}" data-parent="#accordion_lessons">
											<div class="card-body">
												<div class="list_lessons">
													<ul>
														...
														for ($a = $start; $a < $end; $a++) { ?>
															...
															<style>a.disabled {pointer-events: none;} li.disable-links {pointer-events: none;}</style>
															@if(Auth::check())
																@if(auth()->user()->type_user == 'Admin')
																	<?php $filesNotEmpty = 'filesNotEmpty' ?>
																	<li><a href="{{ $arr_video_url[$a] }}" class="video">{{ $arr_video_title[$a] }}</a></li>
																@elseif($price == 0)
																	<?php $filesNotEmpty = 'filesNotEmpty' ?>
																	<li><a href="{{ $arr_video_url[$a] }}" class="video">{{ $arr_video_title[$a] }}</a></li>
																...
																@elseif($course_details->user_id  == auth()->user()->id)
																	...
																	<li><a href="{{ $arr_video_url[$a] }}" class="video">{{ $arr_video_title[$a] }}</a></li>
																@else
																	<li class="disable-links"><a href="" class="video">{{ $arr_video_title[$a] }}</a></li>
																@endif
															@else
																<li class="disable-links"><a href="" class="video">{{ $arr_video_title[$a] }}</a>{{-- <span>00:59</span> --}}</li>
															@endif
								...
								<div class="card">
									<div class="card-header" role="tab" id="heading_files">
										<h5 class="mb-0">
											<a data-toggle="collapse" href="#collapse_files" aria-expanded="true" aria-controls="collapse_files"><i class="indicator ti-plus {{-- minus --}}"></i> Course Files</a>
										</h5>
									</div>

									<div id="collapse_files" class="collapse {{-- show --}}" role="tabpanel" aria-labelledby="heading_files" data-parent="#accordion_lessons">
										<div class="card-body">
											<div class="list_lessons">
												<ul>
													@foreach($courseFiles as $courseFile)
														@if(!empty($filesNotEmpty))
															<li><a href="{{ $courseFile->file_name }}" class="txt_doc">{{ $courseFile->name }}</a><span>{{ EducationAlzardHelpers::bytesToHuman($courseFile->size) }}</span></li>
														@else
															<li class="disable-links"><a href="" class="txt_doc">{{ $courseFile->name }}</a><span>{{ $courseFile->size }}</span></li>
														@endif
													@endforeach
												</ul>
											</div>
						...
						<section id="reviews">
									...
									<div class="col-lg-9">
										<div class="row">
											<div class="col-lg-10 col-9">
												<div class="progress">
													<div class="progress-bar" role="progressbar" style="width: {{ round(($reviews['reviews5']/$reviews['countReviews'])*100) }}%" aria-valuenow="{{ ($reviews['countReviews'] / 5) * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
												</div>
											</div>
										...
										<div class="row">
											<div class="col-lg-10 col-9">
												<div class="progress">
													<div class="progress-bar" role="progressbar" style="width: {{ round(($reviews['reviews4']/$reviews['countReviews'])*100) }}%" aria-valuenow="{{ ($reviews['countReviews'] / 4) * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
												</div>
											</div>
										...
										<div class="row">
											<div class="col-lg-10 col-9">
												<div class="progress">
													<div class="progress-bar" role="progressbar" style="width: {{ round(($reviews['reviews3']/$reviews['countReviews'])*100) }}%" aria-valuenow="{{ ($reviews['countReviews'] / 3) * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
												</div>
											</div>
										...
										<div class="row">
											<div class="col-lg-10 col-9">
												<div class="progress">
													<div class="progress-bar" role="progressbar" style="width: {{ round(($reviews['reviews2']/$reviews['countReviews'])*100) }}%" aria-valuenow="{{ ($reviews['countReviews'] / 2) * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
												</div>
											</div>
										...
										<div class="row">
											<div class="col-lg-10 col-9">
												<div class="progress">
													<div class="progress-bar" role="progressbar" style="width: {{ round(($reviews['reviews1']/$reviews['countReviews'])*100) }}%" aria-valuenow="{{ ($reviews['countReviews'] / 1) * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
												</div>
											</div>
							...
							<div class="reviews-container">
								...
								<div class="review-box clearfix">
									<figure class="rev-thumb"><img src="{{ asset('/uplaod/user/'.$comment[1]['image']) }}" alt="">
									</figure>
									<div class="rev-content">
										<div class="comment_right clearfix">
											<div class="comment_info">
												By <a href="#">{{ $comment[1]['name'] }}</a> <span class="float-right"> {{ \Carbon\Carbon::parse($comment[0]['created_at'])->diffForHumans() }}</span>
											</div>
										</div>
										<div class="rating">
										...
										<div class="rev-info">
											{{ $comment[1]['type_user'] }} – {{ \Carbon\Carbon::parse($comment[0]['created_at'])->toFormattedDateString() }} {{-- - <a href="?reply={{ $comment[1]['id'] }}">Reply</a> --}}
										</div>
										<div class="rev-text">
											<p>
												{!! $comment[0]['comment'] !!} 
									...
									<div class="form-group">
										...
										<div class="star-rating">
											<div class="rating">
											    ...
											    <input type="hidden" id="star_number" name="star_number" class="rating-value" value="4">
											</div>
										</div>
									</div>
									<div class="form-group">
										<textarea class="form-control" name="comment" id="comments2" rows="6" placeholder="Message Below" required=""></textarea>
									</div>
									<div class="form-group">
					...
					<aside class="col-lg-4" id="sidebar">
						<div class="box_detail">
							<figure>
								<a href="{{ $course_details->course_video }}" class="video"><i class="arrow_triangle-right"></i><img src="{{ asset('/uplaod/courses/coursesimages/'. $course_details->course_image) }}" alt="" class="img-fluid"><span>View course preview</span></a>
							</figure>
							<div class="price">
								${{ $course_details->course_price - $course_details->course_discount_price }}<span class="original_price"><em>${{ $course_details->course_discount_price }}</em>{{ explode(".",(($course_details->course_price - $course_details->course_discount_price) * 100)/$course_details->course_price)[0] }}% discount price</span>
							</div>
							...
							@if($price != 0)
								<a href="{{ url('add-to-cart/'.$course_details->id) }}" class="btn_1 full-width outline"><i class="icon_heart"></i> Add to wishlist</a>
							@endif
							{{-- @endif --}}
							<div id="list_feat">
