			...
			<div class="row">
				<div class="col-lg-9">
					<div class="bloglist singlepost">
						...
						<div class="postmeta">
							<ul>
								...
								<li><a href="#"><i class="icon_clock_alt"></i> {{ $post->created_at->toDayDateTimeString() }} </a></li>
								<li><a href="#"><i class="icon_pencil-edit"></i> Admin</a></li>
								<li><a href="#"><i class="icon_comment_alt"></i> ( {{ count($id->blog_comments) }} ) Comments</a></li>
							</ul>
						</div>
						<!-- /post meta -->
						...
						<ul>
							@foreach($id->blog_comments as $comm)
								@if($comm->reply_id == null)
									<li>
										<div class="avatar">
											<a href="#"><img src="{{ asset('design/educationalzard/img/user-image.png') }}" alt="">
											</a>
										</div>
										<div class="comment_right clearfix">
											<div class="comment_info">
												By <a href="{{ $comm->website }}">{{ $comm->name }}</a><span>|</span>{{ $comm->created_at->toDateString() }}<span>|</span><a href="?reply={{ $comm->id }}">Reply</a> <span class="float-right">{{ \Carbon\Carbon::parse($comm->created_at)->diffForHumans() }}</span>
											</div>
											<p>
												{{ $comm->comment }}.
											</p>
										...
										<ul class="replied-to">
											<?php
												...
												for ($i=0; $i < count($comm_son); $i++) { ?>
													...
													<li>
														<div class="avatar">
															<a href="#"><img src="{{ asset('design/educationalzard/img/user-image-1.jpg') }}" alt="">
															</a>
														</div>
														<div class="comment_right clearfix">
															<div class="comment_info">
																By <a href="#">{{ $comm_son[$i]->name }}</a><span>|</span>{{ $comm_son[$i]->created_at }}<span>|</span><a href="?reply={{	$id = $comm_son[$i]->id }}">Reply</a> <span class="float-right">{{ \Carbon\Carbon::parse($comm_son[$i]->created_at)->diffForHumans() }}</span>
															</div>
															...
															<?php
																...
																for ($i=0; $i < count($comm_grandson); $i++) { ?>
																	<li>
																		...
																		<div class="comment_right clearfix">
																			<div class="comment_info">
																				By <a href="#">{{ $comm_grandson[$i]->name }}</a><span>|</span>{{ $comm_grandson[$i]->created_at }}<span>|</span><a href="?reply={{ $id }}">Reply</a> <span class="float-right">{{ \Carbon\Carbon::parse($comm_grandson[$i]->created_at)->diffForHumans() }}</span>
																			</div>
					...
					<form action="/blog/comment-store" method="POST">
						...
						<div class="form-group">
							<input type="text" name="website" id="website3" class="form-control" placeholder="Website" required="">
						</div>
						<div class="form-group">
				...
				<aside class="col-lg-3">
					...
					<div class="widget">
						...
						<ul class="comments-list">
							...
							<li>
								<div class="alignleft">
									<a href="{{ url('/blog/blog-post/'.$post->id) }}"><img src="{{ asset('uplaod/posts/'.$post->image) }}" alt=""></a>
								</div>
								<small>{{ $post->created_at->toDayDateTimeString() }}</small>
								<h3><a href="{{ url('/blog/blog-post/'.$post->id) }}" title="">{!! substr($post->title, 0, 50) !!} ...</a></h3>
							</li>
							@endforeach
						</ul>
					</div>
