...
<header class="header fadeInDown">
    <div id="logo">
        <a href="{{ url('/') }}"><img src="{{ url('/design/educationalzard') }}/img/logo.png" width="149" height="42"
    ...
    <ul id="top_menu">
        ...
        @if(Auth::check())
            @if(auth()->user()->type_user == 'Student')
                <li class="hidden_tablet"><a href="{{ url('/profile/'. auth()->user()->name) }}" class="btn_1 rounded">Go
                        To Profile</a></li>
            @elseif(auth()->user()->type_user == 'Teacher')
                <li class="hidden_tablet"><a href="{{ url('/dashboard/') }}" class="btn_1 rounded">Go To Dashboard</a>
                </li>
